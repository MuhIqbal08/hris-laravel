<div>
    <div class="relative mb-6 w-full">
        ...
        <flux:subheading size="lg" class="mb-6">{{ __('Manage your office location') }}</flux:subheading>
                ...
                <tbody>
                    @foreach ($locations as $location)
                        <tr class="odd:bg-white even:bg-gray-50 border-b border-gray-200">
                            ...
                            <td class="px-6 py-2 text-gray-700">{{ $location->name }}</td>
                            <td class="px-6 py-2 text-gray-700">{{ $location->latitude }}</td>
                            <td class="px-6 py-2 text-gray-700">{{ $location->longitude }}</td>
                            <td class="px-6 py-2 text-gray-700">{{ $location->radius_in_meters }} M</td>
                            <td class="px-6 py-2 space-x-1">
                                @can('location.edit')
                                    <a href="{{ route('location.edit', $location->uuid) }}"
